...
constexpr Vec3<double> position_kp{2.8,2.8,6.0};
constexpr Vec3<double> position_kd{3.2,3.2,4.2};
constexpr Vec3<double> position_ki{1.0,1.0,2.0};
constexpr Vec3<double> attitude_kp{1.30,1.80,3.0};
constexpr Vec3<double> attitude_kd{.40,.58,.95};
...
void BaselineController::reset() { integral_={};previous_={};pending_initialization_=true; }
void BaselineController::reset(const Input<double>& previous) { integral_={};previous_=previous;pending_initialization_=true; }
...
Input<double> BaselineController::command(const State<double>& x,const Reference& ref,
  ...
  if(pending_initialization_) {
    // Reconstruct support from the known held command and noisy rotor state.
    // This initializes the integral continuously at takeover without knowing
    // actual mass or wind; otherwise a nominal-weight step can abort descent.
    auto held=x;for(int j=0;j<NU;++j)held[13+j]=previous_[j];
    const auto support=evaluate(held,previous_,wind,p);
    const auto held_acceleration=subtract(scale(add(rotate(r,support.force_body),support.drag_world),1/p.mass),
                                         Vec3<double>{0,0,p.gravity});
    if(previous_[0]>0)
      for(int j=0;j<3;++j)
        integral_[j]=std::clamp((held_acceleration[j]-ref.acceleration[j]-position_kp[j]*error[j]-
                                position_kd[j]*velocity_error[j])/position_ki[j],-2.0,2.0);
  ...
  // Nominal drag and tail-force feedforward use the supplied measurements.
  // The comparison supplies zero wind to both controllers, not true plant wind.
  ...
  // A static tail map retains the rotor's natural axial damping. Inverting
  // measured inflow and assumed axial flow would cancel that damping with an
  // inaccurate zero-wind estimate, strongly amplifying unknown crosswind.
  Input<double> result{
    inverse_collective(main_thrust,x[17],f.main_axial_airspeed,p.main_radius,p.main_omega,p.main_solidity,p),
    longitudinal,lateral,
    inverse_collective(tail_thrust,std::sqrt(tail_thrust/(2*p.air_density*pi*p.tail_radius*p.tail_radius)),
                       0.0,p.tail_radius,p.tail_omega,p.tail_solidity,p)};
  limited_=false;
  for(int i=0;i<NU;++i) {
    const double requested=result[i];
    ...
    limited_=limited_||std::abs(result[i]-requested)>1e-12;
  }
  previous_=result;
  return result;
}
...
nlohmann::json model_metadata(const Params& p) {
  return {
    ...
    {"baseline",{{"position_kp",position_kp},{"position_kd",position_kd},{"position_ki",position_ki},
                  ...
                  {"integral_initialization","At reset/takeover, reconstruct support from known held pitch and measured rotor state using nominal parameters; initialize bounded position integrals for bumpless force demand"},
                  {"tail_allocation","Steady induced-flow thrust map with zero axial feedforward; retains aerodynamic yaw damping under unmeasured crosswind"},
                  {"description","Cascaded position PID and damped attitude PD with nominal drag, modeled tail-force, gyro/damping feedforward, main rotor inversion and static tail thrust allocation. Paired runs use noisy sensors, nominal mass and zero wind input; identical pitch/rate limits."}}},
    {"equations",{
