...
latexmk -pdf -interaction=nonstopmode -halt-on-error -file-line-error \
  -outdir="$build_dir" helicopter-entropy-optimal-control.tex
qpdf --check "$build_dir/helicopter-entropy-optimal-control.pdf"
pages=$(pdfinfo "$build_dir/helicopter-entropy-optimal-control.pdf" | awk '/^Pages:/ {print $2}')
