...
void value_checkpoint_contract() {
    droid::ConstructionWorld world;
    world.rebuild(droid::ConstructionWorld::light_assembly());
    droid::LightControl controller, baseline;
    controller.reset("learner",59);
    baseline.reset("zero");
    const auto initial=controller.diagnostics();
    rejected([&] { (void)controller.capture_values(); });
    check(controller.diagnostics()==initial,"uninitialized controller capture changed state");
    for (std::size_t step=0; step<32; ++step)
        controller.observe(world.step(controller.act(world.observation())));
    const auto captured=controller.diagnostics();
    const droid::LightControl& read_only=controller;
    const auto checkpoint=read_only.capture_values();
    check(controller.diagnostics()==captured,"LightControl capture changed state");
    const auto baseline_before=baseline.diagnostics();
    rejected([&] { (void)baseline.capture_values(); });
    rejected([&] { baseline.restore_values(checkpoint); });
    check(baseline.diagnostics()==baseline_before,"checkpoint access changed a nonlearning controller");
    const double effort=controller.act(world.observation());
    const auto pending=controller.diagnostics();
    rejected([&] { (void)controller.capture_values(); });
    rejected([&] { controller.restore_values(checkpoint); });
    check(controller.diagnostics()==pending,"pending checkpoint access changed controller state");
    controller.observe(world.step(effort));
    for (std::size_t step=33; step<72; ++step)
        controller.observe(world.step(controller.act(world.observation())));
    const auto before=controller.diagnostics();
    const auto physics=world.state();
    controller.restore_values(checkpoint);
    check(controller.diagnostics().at("parameter_fingerprint_sha256")==captured.at("parameter_fingerprint_sha256"),
          "LightControl did not restore captured value bits");
    check(controller.diagnostics().at("nonparameter_state_fingerprint_sha256")==before.at("nonparameter_state_fingerprint_sha256"),
          "LightControl restore changed wrapper or nonparameter learner state");
    check(world.state()==physics,"value recall altered the world");
    controller.freeze_learning();
    const auto frozen=controller.diagnostics();
    controller.restore_values(checkpoint);
    check(controller.diagnostics()==frozen,"recall changed an otherwise identical frozen control");
    controller.reset("learner",59);
    check(controller.diagnostics()==initial,"LightControl reset retained checkpoint state");
    rejected([&] { controller.restore_values(checkpoint); });
    check(controller.diagnostics()==initial,"uninitialized restore changed reset control");
}
int main() {
    try {
        value_checkpoint_contract();
        droid::ConstructionWorld world;
        world.rebuild(droid::ConstructionWorld::light_assembly());
        ...
        for (const std::string action : {"freeze_learning","capture_values","restore_values"})
            rejected([&] { (void)session.control(Json{{"action",action}}); });
