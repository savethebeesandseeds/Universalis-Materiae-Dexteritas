...
struct BodyProbeController::Impl {
    ...
    [[nodiscard]] std::map<std::string, double> act(
        ...
        const LightSample sample = validate_observation(observation, identities, now_s);
        advance_stage();
        observe(sample);
...
Json body_controller_spec() {
    return Json{{"controller_id", kBodyControllerId},
        {"schema_version", "droid-blocks.body-probe-controller.v1"},
        ...
        {"seeking_rule", "constant bounded effort in each retained local sign; no further learning or moving-source tracking"},
