...
Json validate_assembly(const Json& input) {
    keys(input, {"schema", "name", "segments", "blocks", "sensor"}, "assembly");
    if (input.at("schema") != "construction_kit_v1") {
...
std::string generate_xml(const Json& assembly, std::vector<Geometry>& geometry) {
    ...
    xml.precision(17);
    xml << "<mujoco model=\"construction_kit_v1\">"
    ...
    for (int segment = 0; segment < 2; ++segment) {
        ...
        xml << "<body name=\"segment_" << segment << "\" pos=\"0 0 " << offset << "\">"
            << "<joint name=\"" << (segment == 0 ? "motor_joint" : "passive_joint")
            << "\" type=\"hinge\" axis=\"0 1 0\" limited=\"false\" damping=\""
            << (segment == 0 ? 0.0 : kPassiveDamping) << "\" armature=\""
            << (segment == 0 ? reflected_rotor_inertia() : kPassiveArmature) << "\"/>";
...
struct ConstructionWorld::Impl {
    ...
    explicit Impl(const Json& input) : assembly(validate_assembly(input)) {
        ...
        if (model->nu != 1 || model->nv != 2) {
            throw std::runtime_error("construction must have one actuator and two hinge degrees of freedom");
        }
        reset();
    }
    ...
    Json step(double effort) {
        ...
        return Json{{"observation", observation()}, {"reward", transition_value},
    ...
    Json state() const {
        ...
        return Json{{"schema", "construction_state_v1"}, {"assembly", assembly},
            {"elapsed_s", seconds(tick)}, {"sim_time", seconds(tick)}, {"tick", tick},
            {"geometry", displayed}, {"segments", segments},
            {"joints", Json{{"motor_m", Json::array({0.0, 0.0, 0.0})},
                {"passive_m", values(data->xpos + 3 * body_ids[1])}}},
